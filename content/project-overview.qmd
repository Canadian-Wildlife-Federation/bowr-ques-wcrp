...
gain_bowr <- api$gain_bowr
num_dam_bowr <- api$num_dam_bowr
km_dam_bowr <- api$km_dam_bowr
pct_dam_bowr <- api$pct_dam_bowr
resource_km_bowr <- api$resource_km_bowr
resource_pct_bowr <- api$resource_pct_bowr
demo_km_bowr <- api$demo_km_bowr
demo_pct_bowr <- api$demo_pct_bowr
resource_sev_bowr <- api$resource_sev_bowr
demo_sev_bowr <- api$demo_sev_bowr
sum_road_bowr <- api$sum_road_bowr

gain_ques_carr <- api$gain_ques_carr
num_dam_ques_carr <- api$num_dam_ques_carr
km_dam_ques_carr <- api$km_dam_ques_carr
pct_dam_ques_carr <- api$pct_dam_ques_carr
resource_km_ques_carr <- api$resource_km_ques_carr
resource_pct_ques_carr <- api$resource_pct_ques_carr
demo_km_ques_carr <- api$demo_km_ques_carr
demo_pct_ques_carr <- api$demo_pct_ques_carr
resource_sev_ques_carr <- api$resource_sev_ques_carr
demo_sev_ques_carr <- api$demo_sev_ques_carr
sum_road_ques_carr <- api$sum_road_ques_carr
...
#rating classifier
def rating(threat, barrier, watershed):
    if threat == "extent":
        if barrier == "DAM":
            pct = api.barrier_extent(barrier, watershed)[1]
            rating = condition(pct)
        elif barrier == "ROAD":
            pct = api.barrier_extent('ROAD, RESOURCE/OTHER', watershed)[1] + api.barrier_extent('ROAD, DEMOGRAPHIC', watershed)[1]
            rating = condition(pct)
    elif threat == "severity":
        if barrier == "DAM":
            pct = api.barrier_severity(barrier, watershed)[2]
            rating = condition(pct)
        elif barrier == "ROAD":
            pct = api.barrier_severity('ROAD, RESOURCE/OTHER', watershed)[2] + api.barrier_severity('ROAD, DEMOGRAPHIC', watershed)[2]
            rating = condition(pct)
...
df = pd.DataFrame({"Barrier Types":["Road-Stream Crossings","Lateral Barriers","Small Dams(<3m height)","Trail-stream Crossings", "Natural Barriers"],
                   "Extent":[rating("extent", "ROAD", "BOWR"),"High",rating("extent", "DAM", "BOWR"), "Low", "Medium"],
                   "Severity":[rating("severity", "ROAD", "BOWR"),"Very High",rating("severity", "DAM", "BOWR"), "Low", "High"],
